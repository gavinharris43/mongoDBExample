...
val queryFuture = Future {
 ...
 db.run(peopleTable.result).map(_.foreach {
  ...
  def size={
    val queryFuture = Future {
      db.run(peopleTable.size.result)
    }
    Await.result(queryFuture, Duration.Inf).andThen {
      case Success(value) =>  (println(s"Size of Database: $value"),  averageAge(value))
      case Failure(error) => println("Listing people failed due to: " + error.getMessage)
    }
  }

  def averageAge(size :Int) ={
    val queryFuture = Future {
      db.run(peopleTable.map(_.age).sum.result)
    }
    Await.result(queryFuture, Duration.Inf).andThen {
          // 0 divide by 0 ouch ?
      case Success(value) =>  (println(s"Average Age: ${value.getOrElse(0) / size}"), mostCommonFirstName)
      case Failure(error) => println("Listing people failed due to: " + error.getMessage)
    }
  }
  def mostCommonFirstName: Unit ={
    val queryFuture = Future {
      db.run(peopleTable.map(_.fName).result)
    }
    Await.result(queryFuture, Duration.Inf).andThen {
      case Success(value) =>  (println(s"Most Common First Name: ${value}"), listPeople)
      case Failure(error) => println("Listing people failed due to: " + error.getMessage)
    }
